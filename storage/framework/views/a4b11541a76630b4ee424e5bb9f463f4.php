...
<body class="bg-linear-to-br from-slate-50 via-sky-50/30 to-teal-50/20 flex justify-center items-center min-h-screen p-3 md:p-6 overflow-hidden">
  ...
  <div class="flex flex-col lg:flex-row items-center w-full max-w-(1380px) h-[92vh] max-h-(860px) bg-white/95 backdrop-blur-xl rounded-[28px] shadow-[0_20px_50px_rgba(12,67,83,0.08)] border border-white overflow-hidden animate-fade-in">
    ...
    <div class="hidden lg:flex flex-col justify-between p-10 xl:p-12 w-[42%] h-full relative overflow-hidden bg-[#0C4353]">
      ...
      <img src="<?php echo e(asset('images/paso4.png')); ?>" alt="Tu negocio en orden" class="absolute inset-0 w-full h-full object-cover object-center scale-105 opacity-90 transition-transform duration-700 hover:scale-100" />
      ...
      <div class="w-full max-w-(620px) mx-auto flex flex-col gap-3 h-full">
        ...
        <div class="flex flex-col gap-2 w-full shrink-0">
          <div class="flex justify-between items-center">
            <a href="<?php echo e(route('register.step', ['step' => 3])); ?>" class="flex items-center justify-center w-9 h-9 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-full transition-all duration-200 shadow-sm hover:scale-105 active:scale-95">
        ...
        <form id="regionalForm" action="<?php echo e(route('register.step.store', ['step' => 4])); ?>" method="GET" class="flex flex-col gap-4 w-full overflow-y-auto custom-scroll pr-1 py-1 my-auto">
          ...
          <div class="flex flex-col gap-1.5 w-full">
            ...
            <div class="relative flex items-center">
              ...
              <select id="zona_horaria" name="zona_horaria" required class="w-full h-11 pl-10 pr-10 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 font-medium focus:outline-none focus:border-[#146F8A] focus:ring-2 focus:ring-[#146F8A]/20 transition-all appearance-none cursor-pointer">
                <option value="" disabled <?php echo e(empty($formData['zona_horaria']) ? 'selected' : ''); ?>>Selecciona una zona horaria</option>
                <option value="America/Managua" <?php echo e((isset($formData['zona_horaria']) && $formData['zona_horaria'] == 'America/Managua') || (!isset($formData['zona_horaria']) && config('app.timezone') == 'America/Managua') ? 'selected' : ''); ?>>(UTC-06:00) Managua, Centroamérica</option>
                <option value="America/Mexico_City" <?php echo e((isset($formData['zona_horaria']) && $formData['zona_horaria'] == 'America/Mexico_City') ? 'selected' : ''); ?>>(UTC-06:00) Ciudad de México</option>
                <option value="America/Bogota" <?php echo e((isset($formData['zona_horaria']) && $formData['zona_horaria'] == 'America/Bogota') ? 'selected' : ''); ?>>(UTC-05:00) Bogotá, Lima, Quito</option>
                <option value="America/New_York" <?php echo e((isset($formData['zona_horaria']) && $formData['zona_horaria'] == 'America/New_York') ? 'selected' : ''); ?>>(UTC-05:00) Este de EE. UU. / Canadá</option>
                <option value="Europe/Madrid" <?php echo e((isset($formData['zona_horaria']) && $formData['zona_horaria'] == 'Europe/Madrid') ? 'selected' : ''); ?>>(UTC+01:00) Madrid, España</option>
              </select>
          ...
          <div class="flex flex-col gap-1.5 w-full">
            ...
            <div class="relative flex items-center">
              ...
              <select id="moneda" name="moneda" required class="w-full h-11 pl-10 pr-10 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 font-medium focus:outline-none focus:border-[#146F8A] focus:ring-2 focus:ring-[#146F8A]/20 transition-all appearance-none cursor-pointer">
                <option value="" disabled <?php echo e(empty($formData['moneda']) ? 'selected' : ''); ?>>Tipo de moneda</option>
                <option value="USD" <?php echo e((isset($formData['moneda']) && $formData['moneda'] == 'USD') ? 'selected' : ''); ?>>USD - Dólar estadounidense ($)</option>
                <option value="NIO" <?php echo e((isset($formData['moneda']) && $formData['moneda'] == 'NIO') ? 'selected' : ''); ?>>NIO - Córdoba nicaragüense (C$)</option>
                <option value="EUR" <?php echo e((isset($formData['moneda']) && $formData['moneda'] == 'EUR') ? 'selected' : ''); ?>>EUR - Euro (€)</option>
              </select>
              <span class="absolute right-3.5 text-slate-400 pointer-events-none">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
              </span>
          ...
          <div class="flex flex-col gap-1.5 w-full">
            ...
            <div class="relative flex items-center">
              ...
              <input 
                ...
                id="fecha_creacion" 
                name="fecha_creacion" 
                value="<?php echo e(old('fecha_creacion', $formData['fecha_creacion'] ?? date('Y-m-d'))); ?>"
...
</html><?php /**PATH C:\laragon\www\gintly_app\resources\views/auth/register-step4.blade.php ENDPATH**/ ?>