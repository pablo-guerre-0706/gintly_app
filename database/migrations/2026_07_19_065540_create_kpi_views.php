...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("
            ...
            SELECT business_id,
                SUM(total_amount) AS cartera_emitida,
                SUM(paid_amount)  AS cartera_recuperada,
        ...
        // KPI-02 — Exactitud de stock (bodega vs inventario)
        DB::statement("
            CREATE OR REPLACE VIEW vw_kpi_exactitud_stock AS
            SELECT business_id, DATE(counted_at) AS dia,
                SUM(ABS(difference))     AS desviacion_absoluta,
                SUM(system_quantity)     AS stock_sistema,
        ...
        DB::statement("
            ...
            SELECT a.business_id, DATE(a.detected_at) AS dia,
                SUM(ABS(a.difference)) AS faltante_no_justificado
            FROM anomalies a
            JOIN anomaly_rules r ON r.id = a.anomaly_rule_id
            WHERE r.code = 'faltante_inventario'
                AND a.status NOT IN ('justificada', 'resuelta')
        ...
        DB::statement("
            ...
            SELECT business_id, DATE(started_at) AS dia,
                SUM(status = 'completada') AS corridas_ok,
